    ...
    def generate_scale_chords(
        ...
        for degree_roman, degree_definition in scale_degrees_info.items():
            ...
            if 0 < inversion < len(chord_intervals_relative):
                ...
                chord_intervals_relative = sorted(
                    list(set(temp_intervals))
            ...
            if midi_options["arpeggio_style"]:
                ...
                if num_arp_notes > 0:
                    for idx, note_val in enumerate(arp_notes_sequence):
                        ...
                        if (
                            idx == num_arp_notes - 1
                        ):  # This is the last note of the arpeggio sequence
                            ...
                            current_arp_note_actual_duration = max(
                                0, remaining_slot_time
                            )

                            if (
                                arp_note_indiv_duration_ticks > 0
                                and num_arp_notes * arp_note_indiv_duration_ticks
                                > chord_duration_ticks
                                + (arp_note_indiv_duration_ticks / 2)
                            ):  # Allow some slack
                                print(
                                    f"\033[33mWarning: Arpeggio for chord '{chord_data.get('nombre', '')}' "
                ...
                time_offset_for_strum_completion = (
                    0  # Time from first note_on to last note_on in strum
                )
                ...
                duration_for_first_note_off = (
                    chord_duration_ticks - time_offset_for_strum_completion
                )
                if duration_for_first_note_off < 0:
                    # This means strum is longer than chord duration, which is problematic.
                    print(
                        f"\033[33mWarning: Strum effect for chord '{chord_data.get('nombre', '')}' "
